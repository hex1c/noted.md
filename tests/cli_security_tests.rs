...
fn test_security_migration_command() {
    ...
    let migrated_config: notedmd::config::Config =
        toml::from_str(&migrated_config_content).unwrap();
...
fn test_security_reset_command() {
    ...
    // Verify reset
    assert!(
        !master_key_dir.exists(),
        "Master key file should be deleted"
    );

    let reset_config: notedmd::config::Config =
