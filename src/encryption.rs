...
use argon2::{Argon2, password_hash::{rand_core::RngCore, SaltString}};
...
impl EncryptionData {
    ...
    pub fn decrypt(&self, master_password: &str) -> Result<String, EncryptionError> {
        ...
        let nonce_bytes = STANDARD.decode(nonce_base64)
            .map_err(|e| EncryptionError::DecryptionFailure(e.to_string()))?;            let nonce = Nonce::from_slice(&nonce_bytes);
            if nonce.len() != 12 {
                return Err(EncryptionError::DecryptionFailure("Invalid nonce size".to_string()));
            }
...
impl MasterPassword {
    ...
    pub fn setup(&self, password: &str) -> Result<(), EncryptionError> {
        if let Some(path) = &self.hash_file {
            ...
            Err(EncryptionError::EncryptionFailure("No hash file path configured".to_string()))
        }
    }
    ...
    pub fn verify(&self, password: &str) -> Result<bool, EncryptionError> {
        if let Some(path) = &self.hash_file {
            ...
            let stored_key = STANDARD.decode(stored_key_base64)
                .map_err(|e| EncryptionError::DecryptionFailure(e.to_string()))?;
            ...
            Err(EncryptionError::DecryptionFailure("No hash file path configured".to_string()))
        }
    }
...
pub fn prompt_for_master_password(is_setup: bool) -> Result<String, EncryptionError> {
    ...
    if is_setup {
        ...
        if password != confirm {
            return Err(EncryptionError::EncryptionFailure("Passwords do not match".to_string()));
        }
    }
...
mod tests {
    ...
    fn test_encryption_data_new_and_decrypt() {
        ...
        assert!(matches!(result.unwrap_err(), EncryptionError::InvalidMasterPassword));
    }
